...
flowchart TB
    ...
    classDef connect fill:#FEF3C7,stroke:#F59E0B,stroke-width:2px,color:#92400e

    class FE1,FE2,FE3,FE4,FE5,FE6 frontend
    class BE1,BE2,BE3,BE4,BE5,BE6 backend
    class C1,C2,C3,C4,C5,C6 connect
```
...
sequenceDiagram
    ...
    participant 后端 as ⚙️ 后端

    前端->>后端: POST /api/login<br/>{ username, password }
    Note over 前端: axios.post() 发送请求
