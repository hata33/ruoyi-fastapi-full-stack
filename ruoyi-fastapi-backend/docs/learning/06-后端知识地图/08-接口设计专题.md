...
graph TB
    subgraph API[API根路径 /api]
        ...
        Products --> P1[/:id]
        Products --> P2[/:id/reviews]
        Products --> P3[/category/:categoryId]
    end
...
flowchart TB
    ...
    BatchCreate --> BC[POST /users/batch<br/>body: [{...}, {...}]]
    BatchUpdate --> BU[PUT /users/batch<br/>body: [{id, ...}, {...}]]
    BatchDelete --> BD[DELETE /users/batch<br/>body: [1, 2, 3]]

    BC --> Result[响应: {code, msg, data:<br/>{success: [], failed: []}}]
    BU --> Result
    BD --> Result
...
flowchart TB
    ...
    Choice -->|V1| V1[/api/v1/users]
    Choice -->|V2| V2[/api/v2/users]
