...
```mermaid
flowchart LR
    DO["DO<br>Data Object"] --> DB[数据库表结构]
    VO["VO<br>View Object"] --> API[API 返回结构]
    QM[Query Model] --> REQ[API 请求参数]
...
```mermaid
flowchart TB
    A["前端请求<br>POST /api/user/list<br>{userName: 'admin'}"] --> B["1. Query Model 验证"]
    B --> C["UserQuery<br>userName='admin'"]
    C --> D["2. Service 查询"]
    D --> E["user_do = SysUser"]
    E --> F["3. 转换为 VO"]
    F --> G["user_vo = UserVO"]
    G --> H["4. 返回响应<br>{userId: 1, userName: 'admin'}"]
    H --> I["前端接收<br>JSON 自动序列化"]
