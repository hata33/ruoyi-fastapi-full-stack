...
```mermaid
flowchart TB
    A[URL: /users/123] --> B[匹配路由规则<br/>/users/:id]
    B --> C[渲染组件<br/>UserDetail]
    C --> D[传递参数<br/>{id: '123'}]
...
```mermaid
flowchart TB
    A[URL: /api/user/list] --> B[匹配路由规则<br/>GET /user/list]
    B --> C[调用函数<br/>get_user_list]
    C --> D[返回响应<br/>{"code": 200, ...}]
...
graph TB
    subgraph GET
        G1[参数在 URL 中<br/>?name=admin&amp;page=1]
        G2[有长度限制]
        G3[可被缓存]
        G4[可被收藏为书签]
        G5[幂等: 多次请求结果相同]
