...
| 周次 | 内容 | 项目文档 |
|------|------|----------|
| Week 1-2 | FastAPI 核心知识点系统学习 | `04-FastAPI核心知识点/` |
| Week 3-4 | 理解架构、依赖注入、异步编程 | `start/` + `public/` |
| Week 5-6 | 核心功能：登录、权限、异常处理 | `function/` + `feature/` |

#### FastAPI 核心知识点学习路径（新增）

**系统学习 FastAPI 框架，以项目代码为例进行讲解：**

| 顺序 | 文档 | 内容 |
|------|------|------|
| 1 | [FastAPI 简介与项目结构](./04-FastAPI核心知识点/01-FastAPI简介与项目结构.md) | 框架概述、项目结构、启动流程 |
| 2 | [路由与请求方法](./04-FastAPI核心知识点/02-路由与请求方法.md) | 路由定义、HTTP 方法、参数类型 |
| 3 | [请求参数与验证](./04-FastAPI核心知识点/03-请求参数与验证.md) | Pydantic 模型、验证器、错误处理 |
| 4 | 请求体与 Pydantic 模型 | 嵌套模型、复杂数据验证 |
| 5 | 响应模型与序列化 | 响应格式、数据脱敏、统一响应 |
| 6 | 依赖注入系统 | Depends 使用、类依赖、全局依赖 |
| 7 | 中间件 | 中间件原理、CORS、日志中间件 |
| 8 | 异常处理 | 异常处理器、自定义异常 |
| 9 | 异步编程 | async/await、异步数据库操作 |
| 10 | 安全性与认证 | JWT、OAuth2、权限验证 |
| 11 | 数据库集成 | SQLAlchemy 配置、CRUD 操作 |
| 12 | Redis 缓存 | 缓存操作、缓存策略 |
| 13 | 文件上传处理 | 文件接口、存储策略 |
| 14 | 配置管理 | 环境变量、多环境配置 |
| 15 | 生命周期事件 | 启动/关闭事件、资源管理 |
