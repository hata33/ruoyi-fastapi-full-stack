...
from module_todo.service.note_category_service import NoteCategoryService
from utils.log_util import logger
from utils.page_util import PageResponseModel
from utils.response_util import ResponseUtil


noteCategoryController = APIRouter(prefix='/todo/note/category', dependencies=[Depends(LoginService.get_current_user)])


@noteCategoryController.get(
...
async def add_category(
    request: Request,
    add_category: NoteCategoryModel,
    query_db: AsyncSession = Depends(get_db),
    ...
    add_category_result = await NoteCategoryService.add_category_services(query_db, add_category)
    logger.info(add_category_result.message)

    return ResponseUtil.success(msg=add_category_result.message)


@noteCategoryController.put('', dependencies=[Depends(CheckUserInterfaceAuth('todo:note:category:edit'))])
@ValidateFields(validate_model='edit_category')
@Log(title='记事分类', business_type=BusinessType.UPDATE)
async def edit_category(
    request: Request,
    edit_category: NoteCategoryModel,
    query_db: AsyncSession = Depends(get_db),
    current_user: CurrentUserModel = Depends(LoginService.get_current_user),
):
    """
    ...
    edit_category_result = await NoteCategoryService.edit_category_services(query_db, edit_category)
...
async def delete_category(request: Request, category_id: int, query_db: AsyncSession = Depends(get_db)):
    ...
    删除分类
    """
    delete_category = NoteCategoryModel(categoryId=category_id)
    delete_category_result = await NoteCategoryService.delete_category_services(query_db, delete_category)
